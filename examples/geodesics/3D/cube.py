"""
Cube Lifted Heat Method Example
-------------------------------
Compute lifted heat-method distances on a cube surface.
"""
...
def _pick_source_index(points: np.ndarray, scale: float) -> int:
    """
    Choose a source point near the (+,+,+) corner.
    """
    if points.size == 0:
        raise ValueError("No points available to pick a source.")
    target = np.array([scale, scale, scale], dtype=float)
...
def main():
    ...
    source_index = _pick_source_index(points, scale)
    ...
    ax.set_title("Lifted Heat-Method Distances (Cube)")

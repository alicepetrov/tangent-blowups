"""
Plane Cross Spectral Clustering Example
-----------------------------------------
Compare three approaches to spectral clustering on a plane-cross surface
...
The self-tuning lifted Laplacian is built via the iterated blow-up:
  level 1 (embedded = Chordal-Sasaki: positions + scaled projectors):
          k-NN across the two planes is suppressed where the normal directions
          differ, while the self-tuning bandwidth adapts to local density.

Two planes -> n_clusters = 2.
"""
...
def _normals_to_tangent_frames(normals: np.ndarray) -> np.ndarray:
    ...
    For each normal n, pick the canonical axis least aligned to n, project out n,
    normalise to get t1; then t2 = n x t1 (already unit).
    """
    j    = np.argmin(np.abs(normals), axis=1)          # (N,)
    e    = np.eye(3)[j]                                  # (N, 3)
    proj = np.einsum("ni,ni->n", e, normals)            # (N,)
    t1   = e - proj[:, None] * normals                  # (N, 3)
    t1  /= np.linalg.norm(t1, axis=1, keepdims=True)
    t2   = np.cross(normals, t1)                        # (N, 3)
...
def main() -> None:
    ...
    print(f"Loaded {len(points)} plane-cross surface points.")
    ...
    fig.suptitle(
        f"Plane Cross Spectral Clustering: Euclidean vs Lifted (exact vs PCA normals)\n"

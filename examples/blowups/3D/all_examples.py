...
def build_curve_sample(
    ...
    *,
    ...
    """
    Sample a 3D curve with tangents.
    """
...
def build_surface_sample(
    ...
    *,
    ...
    """
    Sample a 3D surface with normals and return a flattened Sample.
    """
...
def _lift_and_plot_curve(label: str, sample_in: Sample):
    print(f"\n=== Curve: {label} ===")
...
def _lift_and_plot_surface(label: str, sample_in: Sample):
    print(f"\n=== Surface: {label} ===")
    points = np.asarray(sample_in.points, dtype=float)
...
def main():
    ...
    surfaces = [
        ...
        ("Plane-Paraboloid Tangent", plane_paraboloid_tangent(scale_xy=1.2, scale_z=0.25), (0.0, 2.0), (0.0, 1.0)),
        ("Tangent Spheres", tangent_spheres(radius=1.0), (0.0, 2.0), (0.0, 1.0)),
        ("Cube Surface", cube_surface(scale=1.0), (0.0, 6.0), (0.0, 1.0)),
        ("Tetrahedron Surface", tetrahedron_surface(scale=1.2), (0.0, 4.0), (0.0, 1.0)),
    ]

    for name, curve, t_max in curves:
        s = build_curve_sample(curve, n=1500, t_max=t_max)
        _lift_and_plot_curve(name, s)

    for name, surface, u_bounds, v_bounds in surfaces:
        s = build_surface_sample(surface, nu=70, nv=70, u_bounds=u_bounds, v_bounds=v_bounds)
        _lift_and_plot_surface(name, s)
